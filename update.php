...
// Deletes a user, their quiz scores, and quiz history
if (isset($_SESSION['key'])) {
  if (@$_GET['demail'] && $_SESSION['key'] == 'updates') {
    $demail = @$_GET['demail'];
...
// Deletes a quiz and all its questions, options, and answers
if (isset($_SESSION['key'])) {
  if (@$_GET['q'] == 'rmquiz' && $_SESSION['key'] == 'updates') {
    $eid = @$_GET['eid'];
...
if (isset($_SESSION['key'])) {
  if (@$_GET['q'] == 'addquiz' && $_SESSION['key'] == 'updates') {
    $name = $_POST['name'];
...
// Adds questions and options for a quiz to the database
if (isset($_SESSION['key'])) {
  if (@$_GET['q'] == 'addqns' && $_SESSION['key'] == 'updates') {
    $n = @$_GET['n'];
    $eid = @$_GET['eid'];
    $ch = @$_GET['ch'];
    for ($i = 1; $i <= $n; $i++) {
